...
class Game {
    ...
    public Game(ConcurrentMap<String, DataOutputStream> clientOutMap,
                GameServer.ClientSession clientSession) {
    ...
    public void start() {
        try {
            if (clientSession.getIn().readUTF().isEmpty()) {
                sender.sendToClient("용사 " + clientSession.getClientId()
    ...
    private void startStage(GameServer.ClientSession clientSession) {
        ...
        while (!endGame) {
            try {
                if (level == 1 || clientSession.getIn().readUTF().isEmpty()) {
                    stage.start(level);
                    level++;
                }
            } catch (SocketException e) {
